...
@property (nonatomic, assign) XJActionSheetStyle style;
...
#pragma mark - 初始化控件
- (instancetype)initWithTitle:(NSString *)title confirms:(NSArray *)confirms cancel:(NSString *)cancel style:(XJActionSheetStyle)style
{
    self = [super init];
    if (self) {
        self.title = title;
...
#pragma mark - 初始化控件
- (instancetype)initWithTitle:(NSString *)title confirms:(NSArray *)confirms cancel:(NSString *)cancel style:(XJActionSheetStyle)style click:(ClickBlock)click
{
    self = [super init];
    if (self) {
        self.title = title;
...
- (void)createSubViews
{
    ...
    for (int i=0; i<self.confirms.count; i++) {
        ...
        if (self.style == XJActionSheetStyleDestructive) {
            [confirm setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        }else if(self.style == XJActionSheetStyleCancel){
            confirm.titleLabel.font = [UIFont boldSystemFontOfSize:XJSize_17];
        }
